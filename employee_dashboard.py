...
def process_daily_reports(directory):
    ...
    daily_reports = glob.glob(os.path.join(directory, "Daily report_*.xlsx"))
...
def process_new_employees(directory):
    ...
    new_employee_file = glob.glob(os.path.join(directory, "New Employee_*.xlsx"))
...
def create_dashboard():
    ...
    st.write(f"Directory entered: {directory}")
    st.write(f"Path exists: {os.path.exists(directory)}")
